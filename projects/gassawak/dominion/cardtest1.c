/* -----------------------------------------------------------------------
 * cardtest1.c
 * Unit test for smithyFnc() called in cardEffect() in dominion.c, which 
 * should add 3 cards to the currentPlayer's hand from their deck and/or 
...
int main() {
	...
	// ----------- TEST 1: +3 cards to hand --------------
	printf("\nTEST 1: handCount= +3 - discarded, \n");
	...
	// ----------- TEST 2: -3 cards from deck, discardCount should +1 --------------
	printf("\nTEST 2: deckCount -3, discardCount +1\n");
	...
	if(passing == 1)
		printf("\nAll smithy tests passed!\n");
	else
		printf("\nAt least one smithy test failed.\n");
